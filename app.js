...
function downloadPDF() {
    showToast("PDF дайындалуда...", "info");
    const element = document.getElementById('lessonContentDisplay');
...
// ЖАҢА: КҮНДЕЛІКТІ ТАПСЫРМАЛАР (QUESTS)
...
function switchTab(tabId) { ["dashboardView", "lessonView", "examView", "profileView", "leaderboardView"].forEach(id => { document.getElementById(id).classList.add("hidden"); }); document.getElementById(tabId).classList.remove("hidden"); document.getElementById("lessonVideo").src = ""; }
...
async function showLeaderboard() { switchTab('leaderboardView'); const container = document.getElementById('leaderboardContent'); container.innerHTML = "<p>⏳ Рейтинг жүктелуде...</p>"; try { const res = await fetch(`${API_URL}/leaderboard`); const data = await res.json(); if (data.length === 0) { container.innerHTML = "<p>Рейтинг бос.</p>"; return; } let html = `<div class="podium-container">`; if(data[1]) html += `<div class="podium-step second"><div class="podium-avatar">🥈</div><div class="podium-name">${data[1].name}</div><div class="podium-xp">${data[1].xp}</div></div>`; if(data[0]) html += `<div class="podium-step first"><div class="podium-avatar">👑</div><div class="podium-name">${data[0].name}</div><div class="podium-xp">${data[0].xp}</div></div>`; if(data[2]) html += `<div class="podium-step third"><div class="podium-avatar">🥉</div><div class="podium-name">${data[2].name}</div><div class="podium-xp">${data[2].xp}</div></div>`; html += `</div><div class="rank-list">`; for(let i=3; i<data.length; i++) { const isMe = data[i].name === localStorage.getItem("userName") ? "my-rank" : ""; html += `<div class="rank-item ${isMe}"><span><b>${i+1}.</b> ${data[i].name}</span> <b style="color:var(--primary);">${data[i].xp} XP</b></div>`; } html += `</div>`; container.innerHTML = html; } catch (err) { container.innerHTML = "<p style='color:red;'>❌ Қате.</p>"; } }
