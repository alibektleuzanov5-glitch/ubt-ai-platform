...
def generate_lesson(req: LessonRequest, authorization: str = Header(None)):
    prompt = f"'{req.topic}' тақырыбы бойынша оқушыға түсінікті қысқаша конспект, негізгі формулалар (LaTeX) және 1 мысал жазып бер. Markdown форматында қайтар."
    try:
        comp = ai_client.chat.completions.create(messages=[{"role": "system", "content": "Мәтінді әдемілеп бер."}, {"role": "user", "content": prompt}], model="llama-3.1-8b-instant")
...
def generate_quiz(req: QuizRequest, authorization: str = Header(None)):
    prompt = f"'{req.topic}' тақырыбы бойынша 3 тест сұрағын құрастыр. ЖАУАПТЫ ТЕК ҚАТАҢ JSON ФОРМАТЫНДА ҚАЙТАР: [{{\"q\":\"Сұрақ?\",\"options\":[\"1\",\"2\",\"3\",\"4\"],\"ans\":\"Дұрыс\"}}]"
    try:
        comp = ai_client.chat.completions.create(messages=[{"role": "system", "content": "Сен тек JSON қайтарасың."}, {"role": "user", "content": prompt}], model="llama-3.1-8b-instant")
